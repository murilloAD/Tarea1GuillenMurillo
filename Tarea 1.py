...
#se define la funcion de basic_ops que es la encargada de realizar las operaciones que se definiran mas abajo
...
def array_ops(Array1, Array2, OpSelector):  #argumentos de entrada: los dos arreglos y el numero de operacion
    ...
    if len(Array1)!=len(Array2):   #compara el tamaño de los arrays, si no son iguales, termina la funcion
        ...
        while n<len(Array1):   #inicia el ciclo para obtener los números de los arreglos
            if (Array1[n]<128)and(Array2[n]<128):    #comprueba que los números estén dentro de los límites
...
#se crea un loop en caso de que se escoja un numero de operacion incorrecto, para que imprima el menu nuevamente y se pueda escoger correctamente
x = 0
while (x == 0):
    ...
    #Se define un if para el caso de AND y OR por separado, ya que estos no funcionan con floats, necesitan ser int
